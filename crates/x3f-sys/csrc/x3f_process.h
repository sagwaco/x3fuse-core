...
/* Returns the global highlight-recovery scale factor applied in
   apply_highlight_clip_dng to the most recent DNG output (1.0 if
   unscaled). The DNG writer adds log2(scale) to BaselineExposure so
   Lightroom auto-restores brightness compressed into the [0, 1] range
   while preserving recovered highlight chromaticity and detail. */
extern double x3f_get_dng_highlight_scale(void);
