...
- **Byte-identical parity is the gate.** Tier-2 pins metadata/JPEG-thumbnail/PPM hashes automatically; DNG and TIFF have manual `-no-denoise` reference baselines on three corpus files (SD1M DNG `16f0d954…` TIFF `277cf4b4…`, older raw `_SDI8040` DNG `58b0376f…` TIFF `b4cc09aa…`, Quattro `_SDI8284` DNG `7402b517…` TIFF `661df021…`) that must match unless a change is an _intentional_ algorithm change — when it is, re-pin the table in [docs/src/contributing.md](docs/src/contributing.md) in the same commit.
